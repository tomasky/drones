...
void build_boringssl(const char *arch) {
    ...
    run("cd uWebSockets/uSockets/boringssl && mkdir -p %s && cd %s && cmake -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DOPENSSL_NO_ASM=0 -DCMAKE_SYSTEM_PROCESSOR=x86 -DCMAKE_BUILD_TYPE=Release .. && make crypto ssl", arch, arch);
#endif
